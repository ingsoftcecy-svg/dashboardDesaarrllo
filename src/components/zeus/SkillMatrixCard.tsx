...
import { Users, ShieldAlert, BadgeCheck, Leaf, Wrench, Search } from "lucide-react";
      ...
      <header className="flex flex-col gap-3 border-b border-slate-100 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-900">
            {filteredOperadores.length} / {operadores.length}
          </span>
        </div>
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] text-slate-500 line-clamp-2" title={op.puesto}>{op.puesto}</div>
                  {(op.equipoAutonomo || op.lider || (op.equipos && op.equipos.length > 0)) && (
                    <div className="mt-1 flex flex-col gap-0.5 text-[9px] leading-tight text-slate-400">
                      {op.equipoAutonomo && <span>EA: <span className="font-medium text-slate-500">{op.equipoAutonomo}</span></span>}
                      {op.lider && <span>Líder: <span className="font-medium text-slate-500">{op.lider}</span></span>}
                      {op.equipos && op.equipos.length > 0 && <span className="line-clamp-2" title={op.equipos.join(", ")}>Equipos: <span className="font-medium text-slate-500">{op.equipos.join(", ")}</span></span>}
                    </div>
                  )}
                </div>
