...
function CargarDatos() {
  ...
  const [seccionActiva, setSeccionActiva] = useState<'autonomia' | 'cursos' | 'operadores'>('autonomia');
      ...
      <main className={cn("mx-auto p-6 mt-4", seccionActiva === 'operadores' ? "max-w-7xl" : "max-w-4xl")}>
        ...
        {seccionActiva === 'autonomia' ? (
          <div className="space-y-6 animate-fade-in">
            ...
            {/* SECCIÓN B: EDITOR EN VIVO DE AUTONOMÍA */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-[#1a4491] px-6 py-3.5 flex items-center gap-3 border-b border-blue-900 text-white">
                ...
                <h2 className="text-xs font-black uppercase tracking-wider">
                  Matriz Interactiva de Autonomía
                </h2>
              </div>

              <div className="p-6 space-y-6">
                {/* Selectores de Semana, Departamento y Equipo */}
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <div className="space-y-1">
                    <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider">Semana a Editar</label>
                    <div className="flex gap-2">
                      <select
                        ...
                        className="flex-1 px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 bg-white focus:outline-none"
                      >
                      ...
                      <button
                        onClick={() => setMostrarModalNuevaSemana(true)}
                        title="Crear Nueva Semana"
                        className="px-2.5 py-1.5 bg-[#1a4491] hover:bg-blue-800 text-white text-xs font-black rounded-lg transition"
                      >
                  ...
                  <div className="space-y-1">
                    <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider">Departamento</label>
                    <select
                      value={selectedDepto}
                      onChange={(e) => {
                      ...
                      className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 bg-white focus:outline-none"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider">Equipo Autónomo</label>
                    <select
                  ...
                  <div className="flex items-end">
                    <button
                      onClick={handleInicializarSemana}
                      disabled={!semanaSeleccionada || selectedEquipo === "Todos"}
                      className="w-full py-2 bg-slate-800 hover:bg-slate-900 text-white font-black text-[10px] uppercase tracking-wider rounded-lg transition disabled:opacity-40"
                    >
                      ⚡ Inicializar Semana
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {/* Columna Izquierda: BPRE del equipo y Lista de Colaboradores */}
                  <div className="md:col-span-1 border-r pr-0 md:pr-6 space-y-6">
                    {/* BPRE Form (solo si hay equipo específico seleccionado) */}
                    {selectedEquipo !== "Todos" && (
                      <div className="bg-slate-50 p-4 border rounded-xl space-y-3">
                        <div className="flex justify-between items-center border-b pb-1">
                          <h4 className="text-[10px] font-black text-slate-700 uppercase tracking-wider">BPRE del Equipo</h4>
                          <button
                            onClick={handleSaveBpre}
                            disabled={guardandoBpre || !semanaSeleccionada}
                            className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-black text-[9px] uppercase tracking-wider px-2.5 py-1 rounded transition"
                          >
                            {guardandoBpre ? 'Guardando...' : '💾 Guardar BPRE'}
                          </button>
                        </div>

                        <div className="grid grid-cols-2 gap-2 text-[10px]">
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Dinamica</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreDinamica} onChange={e => setBpreDinamica(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Liderazgo</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreLiderazgo} onChange={e => setBpreLiderazgo(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">SKAP</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreSkap} onChange={e => setBpreSkap(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">ATO</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreAto} onChange={e => setBpreAto(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Seguridad</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreSeguridad} onChange={e => setBpreSeguridad(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Calidad (QUAS)</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreQuas} onChange={e => setBpreQuas(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Multihab</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreMultihab} onChange={e => setBpreMultihab(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">VPO</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreVpo} onChange={e => setBpreVpo(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Solución Prob</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreSolucion} onChange={e => setBpreSolucion(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Infraest</label>
                            <input type="number" min="0" max="4" step="0.1" value={bpreInfraest} onChange={e => setBpreInfraest(Number(e.target.value))} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Fase Actual</label>
                            <select value={bpreFaseActual} onChange={e => setBpreFaseActual(e.target.value)} className="w-full p-1 border rounded bg-white font-bold text-[10px] animate-fade-in">
                              <option value="F1">Fase 1 (F1)</option>
                              <option value="F2">Fase 2 (F2)</option>
                              <option value="F3">Fase 3 (F3)</option>
                              <option value="F4">Fase 4 (F4)</option>
                            </select>
                          </div>
                          <div className="space-y-0.5">
                            <label className="text-[8px] font-black text-slate-500 uppercase">Fecha Compromiso</label>
                            <input type="text" placeholder="SEPTIEMBRE" value={bpreFechaCompromiso} onChange={e => setBpreFechaCompromiso(e.target.value)} className="w-full p-1 border rounded bg-white font-bold animate-fade-in" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      <h4 className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Integrantes del Equipo</h4>
                      <div className="h-[250px] overflow-y-auto divide-y border rounded-lg bg-slate-50/50">
                        {operatorsInSelectedTeam.map((op) => {
                          const rec = getOperatorWeekRecord(op.id);
                          const total = rec ? Math.round((rec["Autonomy Score"] || 0) * 100) : 0;
                          const evaluado = !!rec;

                          return (
                            <button
                              key={op.id}
                              onClick={() => handleSelectAutonomiaOperator(op)}
                              className={cn(
                                "w-full text-left p-3 text-xs flex flex-col gap-0.5 hover:bg-slate-100 transition-colors focus:outline-none relative",
                                selectedAutonomiaOperator?.id === op.id && "bg-blue-50 hover:bg-blue-100 font-bold border-l-4 border-l-[#1a4491]"
                              )}
                            >
                              <span className="font-bold text-slate-900 uppercase truncate leading-tight pr-16">{op.name}</span>
                              <span className="text-[9px] font-black text-[#1a4491] uppercase tracking-widest">{op.id}</span>
                              
                              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                {!evaluado ? (
                                  <span className="text-[8px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded uppercase">Sin Eval.</span>
                                ) : (
                                  <span className={cn(
                                    ...
                                    total >= 50 ? "bg-amber-100 text-amber-800" :
                        ...
                        {operatorsInSelectedTeam.length === 0 && (
                          <div className="p-8 text-center text-slate-400 text-[10px] uppercase font-bold">
                            Sin operarios en este filtro
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Columna Derecha: Edición de Calificaciones del Operador */}
                  <div className="md:col-span-2 space-y-4">
                    {!selectedAutonomiaOperator ? (
                      <div className="h-full flex flex-col items-center justify-center text-slate-400 py-16 space-y-2 border-2 border-dashed border-slate-200 rounded-xl">
                        <span className="text-xs font-black uppercase tracking-wider">Selecciona un Colaborador</span>
                        <p className="text-[10px] text-slate-400 max-w-xs text-center">Haz clic en un integrante de la lista para editar sus calificaciones de autonomía.</p>
                      </div>
                    ) : (
                      <div className="space-y-4 flex flex-col h-full bg-slate-50/50 p-4 border rounded-xl animate-fade-in">
                        <div className="flex justify-between items-center border-b pb-3">
                          <div>
                            <span className="text-[8px] font-black text-[#1a4491] uppercase tracking-widest">Colaborador Seleccionado</span>
                            <h4 className="text-sm font-black text-slate-800 uppercase leading-tight">{selectedAutonomiaOperator.name}</h4>
                            <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Sharp ID: {selectedAutonomiaOperator.id} — {selectedAutonomiaOperator.puesto}</span>
                          </div>
                          <button
                            onClick={handleSaveAutonomiaOperator}
                            disabled={guardandoAutonomiaOp || !semanaSeleccionada}
                          ...
                          >
                            {guardandoAutonomiaOp ? 'Guardando...' : '💾 Guardar Evaluación'}
                          </button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider">Evaluador</label>
                            <input
                              type="text"
                              value={autonomiaEvaluator}
                              onChange={e => setAutonomiaEvaluator(e.target.value)}
                              className="w-full rounded-lg bg-white px-3 py-2 text-xs font-bold text-slate-800 outline-none border border-slate-200 focus:border-[#1a4491] transition-all"
                              placeholder="Nombre del Evaluador"
                            />
                          </div>
                          <div className="space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-wider">Fecha de Evaluación</label>
                            <input
                              type="date"
                              value={autonomiaAssessmentDate}
                              onChange={e => setAutonomiaAssessmentDate(e.target.value)}
                              className="w-full rounded-lg bg-white px-3 py-2 text-xs font-bold text-slate-800 outline-none border border-slate-200 focus:border-[#1a4491] transition-all"
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 pt-4 border-t border-slate-200">
                          <h5 className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Porcentajes de Cumplimiento General</h5>

                          {/* Básico */}
                          <div className="space-y-1 bg-white p-3 rounded-lg border">
                            <div className="flex justify-between text-xs font-bold uppercase">
                              <span className="text-slate-600">Nivel Básico (Driver's License)</span>
                              <span className="text-[#1a4491] font-black">{autonomiaBasicScore}%</span>
                            </div>
                            <input
                              type="range"
                              min="0"
                              max="100"
                              value={autonomiaBasicScore}
                              onChange={e => setAutonomiaBasicScore(Number(e.target.value))}
                              className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[#1a4491]"
                            />
                          </div>
                          ...
                          <div className="space-y-1 bg-white p-3 rounded-lg border">
                            <div className="flex justify-between text-xs font-bold uppercase">
                              <span className="text-slate-600">Nivel Intermedio (Capabilities)</span>
                              <span className="text-[#1a4491] font-black">{autonomiaIntermediateScore}%</span>
                            </div>
                            <input
                              type="range"
                              min="0"
                              max="100"
                              value={autonomiaIntermediateScore}
                              onChange={e => setAutonomiaIntermediateScore(Number(e.target.value))}
                              className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[#1a4491]"
                            />
                          </div>
                          ...
                          <div className="space-y-1 bg-white p-3 rounded-lg border">
                            <div className="flex justify-between text-xs font-bold uppercase">
                              <span className="text-slate-600">Nivel Avanzado (Capabilities)</span>
                              <span className="text-[#1a4491] font-black">{autonomiaAdvancedScore}%</span>
                            </div>
                            <input
                              type="range"
                              min="0"
                              max="100"
                              value={autonomiaAdvancedScore}
                              onChange={e => setAutonomiaAdvancedScore(Number(e.target.value))}
                              className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[#1a4491]"
                            />
                          </div>

                          {/* Score Calculado */}
                          <div className="bg-[#1a4491]/5 border border-[#1a4491]/20 rounded-xl p-3.5 flex justify-between items-center">
                            <div>
                              <span className="text-[8px] font-black text-[#1a4491] uppercase tracking-widest">Autonomía Calculada</span>
                              <p className="text-[9px] text-slate-400 font-bold uppercase font-sans">Fórmula: (Básico * 0.5) + (Intermedio * 0.35) + (Avanzado * 0.15)</p>
                            </div>
                            <span className="text-lg font-black text-[#1a4491]">
                              {((autonomiaBasicScore * 0.5) + (autonomiaIntermediateScore * 0.35) + (autonomiaAdvancedScore * 0.15)).toFixed(1)}%
                            </span>
                          </div>
                        </div>
                      </div>
            ...
            {mostrarModalNuevaSemana && (
              <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl border shadow-2xl max-w-sm w-full overflow-hidden animate-scale-in">
                  <div className="bg-[#1a4491] px-5 py-3 text-white flex justify-between items-center">
                    <h3 className="text-xs font-black uppercase tracking-wider">Crear Nueva Semana</h3>
                    <button onClick={() => setMostrarModalNuevaSemana(false)} className="text-white hover:text-slate-200 font-bold text-xs focus:outline-none">&times;</button>
                  </div>
                  <div className="p-5 space-y-4">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-black text-slate-500 uppercase tracking-wider">Identificador de Semana</label>
                      <input
                        type="text"
                        placeholder="ej: 2026-W30"
                        value={nuevaSemanaInput}
                        onChange={e => setNuevaSemanaInput(e.target.value)}
                        className="w-full rounded-lg bg-slate-50 px-3 py-2 text-xs font-bold text-slate-800 outline-none border border-slate-200 focus:border-[#1a4491] focus:bg-white transition-all font-mono"
                      />
                      <p className="text-[8px] text-slate-400 uppercase font-bold">Formato requerido: AAAA-WSS (4 dígitos de año, guion, W, y 2 dígitos de semana)</p>
                    </div>
                    <div className="flex gap-3 justify-end pt-2">
                      <button
                        ...
                        className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs uppercase rounded-lg transition"
                      >
                        Cancelar
                      </button>
                      <button
                        onClick={handleCrearNuevaSemana}
                        className="px-4 py-2 bg-[#ffcc00] hover:bg-amber-400 text-[#1a4491] font-black text-xs uppercase rounded-lg transition shadow-sm"
                      >
                        Crear
                      </button>
                    </div>
